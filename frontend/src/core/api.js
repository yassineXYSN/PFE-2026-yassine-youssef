...
let candidateProfileCache = {
    userId: null,
    value: null,
    expiresAt: 0,
    promise: null,
};
let candidateDashboardSummaryCache = {
    userId: null,
    value: null,
    expiresAt: 0,
    promise: null,
...
const resetCandidateDashboardSummaryCache = () => {
    candidateDashboardSummaryCache = {
        userId: null,
        value: null,
        expiresAt: 0,
        promise: null,
...
export function invalidateCandidateProfileCache() {
    resetCandidateProfileCache();
}

export function invalidateCandidateDashboardSummaryCache() {
...
export async function getCandidateProfile({ forceRefresh = false } = {}) {
    ...
    if (
        ...
        && candidateProfileCache.expiresAt > Date.now()
    ) {
    ...
    candidateProfileCache.promise = apiFetch('/candidat/profile')
        ...
        .finally(() => {
            candidateProfileCache.promise = null;
        });
...
export async function getCandidateDashboardSummary({ forceRefresh = false } = {}) {
    ...
    if (
        !forceRefresh
        && candidateDashboardSummaryCache.value
        && candidateDashboardSummaryCache.expiresAt > Date.now()
    ) {
    ...
    candidateDashboardSummaryCache.promise = apiFetch('/applications/dashboard-summary')
        ...
        .finally(() => {
            candidateDashboardSummaryCache.promise = null;
        });

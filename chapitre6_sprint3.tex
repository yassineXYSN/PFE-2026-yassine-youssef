...
\begin{itemize}
    \item \textbf{Ollama / HuggingFace / OpenAI — LLM configurable par variables d'environnement} : L'ensemble des appels LLM est orchestré par une couche d'abstraction (\texttt{ai\_settings.py}) pilotée exclusivement par des variables d'environnement, sans modification du code. Chaque capacité dispose de sa propre paire de variables : \texttt{QUIZ\_GENERATION\_PROVIDER} et \texttt{QUIZ\_GENERATION\_MODEL\_LOCAL} pour la génération de quiz, \texttt{PROFILE\_ANALYSIS\_PROVIDER} pour le scoring candidat, \texttt{QUIZ\_ANALYSIS\_PROVIDER} pour l'analyse post-quiz. Ollama (\texttt{qwen3:8b} par défaut) est le fournisseur local ; HuggingFace (\texttt{Qwen2.5-72B-Instruct}) et OpenAI sont activables sans déploiement supplémentaire. Un mode \texttt{mock} permet d'exécuter les tests automatisés sans infrastructure IA.
    \item \textbf{MongoDB Atlas Vector Search} : La recherche vectorielle est intégrée aux collections MongoDB, ce qui simplifie l'architecture en conservant les profils, offres, chunks documentaires et embeddings dans le même système de persistance. L'index vectoriel est configurable via \texttt{QUIZ\_VECTOR\_INDEX\_NAME}.
    \item \textbf{\texttt{nomic-embed-text} (via Ollama)} : Modèle d'embedding open-source produisant des vecteurs de 768 dimensions à partir des profils, offres et chunks de documents. Substitutable via la variable \texttt{QUIZ\_EMBEDDING\_MODEL} ou \texttt{PROFILE\_ANALYSIS\_EMBEDDING\_MODEL}.
    \item \textbf{tiktoken} : Bibliothèque de tokenisation (encodage \texttt{cl100k\_base}) utilisée pour le comptage précis des tokens lors du découpage en chunks, garantissant des fragments de taille homogène indépendamment de la langue du document.
    ...
    \item \textbf{PyMuPDF et OCR fallback} : Le parsing de CV et de documents s'appuie sur une extraction textuelle robuste (PyMuPDF), complétée par un fallback OCR pour les fichiers scannés.
\end{itemize}
...
La figure~\ref{fig:ui_candidate_recommendations} présente le tableau de bord de recommandations du candidat, qui centralise les offres suggérées par l'IA (calculées à partir de son profil vectorisé) et l'analyse de ses compétences pour orienter sa progression de carrière.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \fbox{\begin{minipage}[c][5cm][c]{\linewidth}
            \centering
            \texttt{sprint3\_ui\_candidate\_job\_recommendations.png}\\[6pt]
            \small\textit{— Capture à insérer —}
        \end{minipage}}
        \caption{Recommandations d'offres personnalisées (Candidat)}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \fbox{\begin{minipage}[c][5cm][c]{\linewidth}
